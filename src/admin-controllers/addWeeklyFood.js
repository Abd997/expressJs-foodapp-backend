  ...
  try {
    ...
    custom = custom.map((element) => {
      return {
        ...
        price: parseInt(element.split("-")[1]),
		protein: parseInt(element.split("-")[2]),
		fat: parseInt(element.split("-")[3]),
		calories: parseInt(element.split("-")[4]),
		carbs: parseInt(element.split("-")[5]),	
      };
    });
    ...
    res.send({
      ...
      data: await FoodCollection.findById({ _id: result._id }).populate({
        path: "ingredients",
