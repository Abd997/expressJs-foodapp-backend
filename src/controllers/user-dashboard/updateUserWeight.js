...
module.exports = async (req, res) => {
  try {
    ...
    let { age, height, weightGoal, currentActivityLevel, gender } =
      user_details;

    let bmi = 0;
    if (weight.unit == "kg" && height.unit == "cm") {
      bmi = weight.value / ((height.value / 100) * (height.value / 100));
    } else if (weight.unit == "kg" && height.unit == "ft") {
      bmi = weight.value / (height.value * 0.3048 * (height.value * 0.3048));
    } else if (weight.unit == "lb" && height.unit == "in") {
      bmi = (703 * weight.value) / (height.value * height.value);
    } else if (weight.unit == "lb" && height.unit == "ft") {
      let heightValue =
        parseInt(height.value.toString().split(".")[0] * 12) +
        parseInt(height.value.toString().split(".")[1]);
    ...
    if (gender == "male") {
      ...
      bmr = 655.1 + 9.563 * newWeight + 1.85 * newHeight - 4.676 * age;
    }
